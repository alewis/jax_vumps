...
import bhtools.tebd.writers as writers
...
def make_writer(observables = None, outdir=None, 
        extra_headers=["E", "Loss", "mpsnorm", "EnergyVariance"]):
    """
    Initialize the writer.
    """
    if outdir is None:
        return None
    headers = [name for name, obs, verbose in observables]
    headers += extra_headers
    writer = writers.writer(outdir, headers=headers, timestamp=False)
    return writer
...
def maximize_overlap(thatmpslist, chi, thatAC=None, tol=1E-13):
    ...
    lam, L = tmeigs(thatAL, B=np.conj(AL), tol=0.01, 
            which="LM", direction="left")
    _, R = tmeigs(thatAR, B=np.conj(AR), tol=0.01, 
            which="LM", direction="right")
    ...
    while delta >= tol:
        print(delta)
        lam, L = tmeigs(thatAL, B=np.conj(AL), tol=delta/10, 
                which="LM", direction="left")
        _, R = tmeigs(thatAR, B=np.conj(AR), tol=delta/10, 
                which="LM", direction="right")
        AC = ct.gauge_transform(L, thatAC, R.T)
        C = ct.gauge_transform(L, thatC, R.T)
        print(np.diag(C))
...
# def vumps_loss(mpslist, Hlist):
    # """
    # The vumps loss function:
    # delta = ||H_Ac(Ac) - A_L . H_C(C)||. 
    # This is zero when the three equations defining the VUMPS 
    # gradient descent are simultaneously satisfied. 
    # """
    # A_L, C, A_R = mpslist
    # A_C = ct.rightmult(A_L, C)
    # term1 = apply_HAc(A_C, A_L, A_R, Hlist)
    # #print("term1 ", term1)
    # term2 = apply_Hc(C, A_L, A_R, Hlist)
    # term2 = ct.rightmult(A_L, term2)
    # #print("term2 ", term2)
    # loss_vector = term1 - term2
    # loss = np.linalg.norm(loss_vector.flatten())
    # return loss
...
# def vumps_iteration_multicell(mpslist, Hchain, delta,
        # params, vumps_state):
    # #N = params["unit_cell"]
    # A_Ls, Cs, A_Rs = mpslist
    # SVD_mode, A_Cs, lhs, rhs = vumps_state
    # # A_Ls = []
    # # Cs = []
    # # A_Rs = []
    # for n, H in enumerate(Hchain):
        # #CL_idx = n-1 #Note this wraps around correctly!
        # oldA_L = copy.deepcopy(A_Ls[n])
        # cell_list = [A_Ls[n], Cs[n], A_Rs[n]]
        # rL, lR = vumps_tm_eigs(cell_list)

        # enviro_params = extract_enviro_params(params, delta)
        # LH = solve_for_LH(A_Ls[n], H, lR, enviro_params, oldLH=lhs[n])
        # RH = solve_for_RH(A_Rs[n], H, rL, enviro_params, oldRH=rhs[n])
        # Hlist = [H, LH, RH]

        # Heff_params = extract_Heff_params(params, delta)
        # A_Cs[n] = minimize_HAc(mpslist, A_Cs[n], Hlist, Heff_params)
        # C_L = minimize_Hc(mpslist, Hlist, Heff_params)
        # C_R = minimize_Hc(mpslist, Hlist, Heff_params)

        # gauge_thresh = params["svd_switch"]
        # A_L, _, SVD_mode = gauge_match(A_Cs[n], C_R, SVD_mode, gauge_thresh)
        # _, A_R, SVD_mode = gauge_match(A_Cs[n], C_L, SVD_mode, gauge_thresh)
        # delta = vumps_loss(oldA_L, A_Cs[n])

        # A_Ls[n] = A_L
        # A_Rs[n] = A_R
        # Cs[n-1] = C_L #Note this wraps around correctly! 
        # Cs[n] = C
...
# def vumps_environment(mpslist, fpoints, H, delta, params,
    ...
    # if params["adaptive_tm_tol"]:
        # tol *= delta 
    # fpoints = vumps_tm_eigs(mpslist, params, oldevs=fpoints,
            # tol=tol)
...
def vumps(H,  
    ...
    while delta >= tol and Niter < max_iter:
        ...
        # if (params["dynamic_chi"] and Niter % params["checkchi"] == 0
                # and B2>params["B2_thresh"]):
            # raise NotImplementedError("Dynamic bond dimension doesn't work yet.")
            # oldchi = A_L.shape[1]
            # newchi = A_L.shape[1] + params["dchi"]
            # E = twositeexpect(mpslist, H, divide_by_norm=False)
            # print("before: ", E)
            # vumps_print(writer, 
                # "Increasing bond dimension from " + str(oldchi) + 
                # " to " + str(newchi))
            # vumps_print(writer, "Checkpointing first.")
            # E = twositeexpect(mpslist, H, divide_by_norm=False)
            # allout = [mpslist, A_C, H_env]
            # writer.pickle(allout, Niter)

            # mpslist, A_C, fpoints, H_env = subspace_expansion(oldlist,
                    # mpslist, params["dchi"], H, H_env, fpoints, delta,
                    # params)
    ...
    #mps = iMPS_from_mixed_canonical(AL, AR, C)
    #    writer.checkpoint(mps, Niter)
